...
#
#  Related Topics Array Dynamic Programming Greedy 👍 5001 👎 159
...
class Solution:
    def wiggleMaxLength(self, nums: List[int]) -> int:
        ...
        prediff = 0
        curdiff = 0
        result = 1  # default start with 1

        for i in range(0, len(nums) - 1):  # until last but one
            # calculate the current diff
            curdiff = nums[i+1] - nums[i]

            # if increase/decreasing, mark the fluctuation
            # wiggle means /\ up down or \/ but not /- or -/
            if (prediff >= 0 and curdiff < 0) or (prediff <= 0 and curdiff > 0):
                print(curdiff)
                result += 1
                # only wiggle happens , can get rid of the plateau
                # go to next wave
                prediff = curdiff
