...
character replaced with the character 3 digits higher (wrapping around from 9 to 0).
...
def rotationalCipher(input_str: str, rotation_factor: int):
    # Write your code here
    def rotate_char(c):
        if "A" <= c <= "Z":  # Check if character is uppercase
            return chr((ord(c) - ord("A") + rotation_factor) % 26 + ord("A"))
        elif "a" <= c <= "z":  # Check if character is lowercase
            return chr((ord(c) - ord("a") + rotation_factor) % 26 + ord("a"))
        elif "0" <= c <= "9":  # Check if character is number
            return chr((ord(c) - ord("0") + rotation_factor) % 10 + ord("0"))
        return c  # Non-alphabet characters are returned as is

    # Apply rotation to each character and join the result
    return "".join([rotate_char(c) for c in input_str])
