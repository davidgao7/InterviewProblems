...
class Solution:
    def findCheapestPrice(
        ...
        # bellman-ford algorithm O(E * k)
        prices = [float("inf")] * n
        prices[src] = 0

        for _ in range(k + 1):
            # create a copy of prices for temporary storage for updates
            temp = prices.copy()
            ...
            for s, d, p in flights:
                ...
                p: price
                """
                # if the destination if impossible to reach, continue
                if prices[s] == float("inf"):
                    continue
                # if the current price(price to reach s + current price) is less than the current price
                if prices[s] + p < temp[d]:  # use temp to prevent overwritiing price
                    # update the price to be the new minimum price
                    temp[d] = prices[s] + p

            prices = temp

        # reutrn the price to reach the destination
